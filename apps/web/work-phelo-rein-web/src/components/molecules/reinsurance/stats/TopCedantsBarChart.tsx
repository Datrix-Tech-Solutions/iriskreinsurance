...
interface TopCedantsBarChartProps {
  title: string;
  /** Already ranked (highest first) and capped to the bars to render. */
  rows: TopCedantRow[];
  isLoading: boolean;
  emptyMessage: string;
  /** Singular noun for a bar's value, e.g. `'offer'` → "3 offers" / "1 offer". */
  valueNoun: string;
  /** Row label for the count line in the hover card, e.g. `'Offers'`. */
  countLabel: string;
  /** Overrides the card's default height (`h-72`). */
  className?: string;
}

export function TopCedantsBarChart({
  title,
  rows,
  isLoading,
  emptyMessage,
              ...
              series={[
                {
                  ...
                  valueFormatter: (value) =>
                    `${value} ${value === 1 ? valueNoun : `${valueNoun}s`}`,
                },
              ]}
              yAxis={[
                {
